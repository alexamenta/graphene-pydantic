...
class PydanticInterfaceTypeOptions(InterfaceOptions):
    pass


def construct_fields(
    interface_type: T.Type["PydanticInterfaceType"],
    ...
    """
    Construct all the fields for a PydanticInterfaceType.
    ...
    for name, field in fields_to_convert:
        converted = convert_pydantic_field(
            name, field, registry, parent_type=interface_type, model=model
        )
        registry.register_object_field(interface_type, name, field)
        fields[name] = converted
    return fields


class PydanticInterfaceType(graphene.Interface):
    ...
    def __init_subclass_with_meta__(
        ...
        **options,
        ...
        if not registry:
            registry = get_global_registry(PydanticInterfaceType)
        ...
        if not _meta:
            _meta = PydanticInterfaceTypeOptions(cls)
